...
def branch_diversity_routine(k, n, SNR_TRAIN, channelDist='rayleigh'):
    ...
    if channelDist == 'rayleigh':
        ...
        lmbda = 1
        fade_taps_I = (np.random.exponential(lmbda, (training_set_size, n)) - np.random.exponential(lmbda, (training_set_size, n))) 
        fade_taps_I /= np.sqrt(2 * np.var(fade_taps_I))
        fade_taps_Q = (np.random.exponential(lmbda, (training_set_size, n)) - np.random.exponential(lmbda, (training_set_size, n))) 
        fade_taps_Q /= np.sqrt(2 * np.var(fade_taps_Q))
    elif channelDist == 'gamma':
        k_param = 3
        theta = np.sqrt(0.5 / k_param)
        fade_taps_I = np.random.gamma(k_param, theta, (training_set_size, n))
        fade_taps_Q = np.random.gamma(k_param, theta, (training_set_size, n))
    ...
    autoencoder.save(f'models/modelSISOnoCSI_uncoded_{k}_{n}_{channelDist}.keras')
    encoder_model.save(f'models/encoder_model_uncoded_{k}_{n}_{channelDist}.keras')
    decoder_model.save(f'models/decoder_model_uncoded_{k}_{n}_{channelDist}.keras')
    ...
    with open(f'numerical_results/results_SISOnoCSI_uncoded_{k}_{n}_{channelDist}.txt', mode='w') as file_id:
    ...
    fig.savefig(f'figures/ber_SISO_noCSI_uncoded_{k}_{n}_{channelDist}.png')
    with open(f'numerical_results/results_SISOnoCSI_uncoded_{k}_{n}_{channelDist}.txt', mode='a') as file_id:
        file_id.write(f'BLER_uncoded_ortho = {BLER_uncoded_ortho}\n')
        file_id.write(f'BLER_coded_ortho_hard = {BLER_coded_ortho_hard}\n')
        file_id.write(f'BLER_learned_uncoded_{k}_{n}_{channelDist} = {BLER_learned_uncoded}\n')
        file_id.write(f'----------------------------------------------------\n')
    
    # Mappings learnt:
    all_possible_inputs = np.eye(M)
    codebook = encoder_model.predict(all_possible_inputs)
    with open(f'numerical_results/encodings_{k}_{n}_{channelDist}.txt', mode='w') as file_id:
