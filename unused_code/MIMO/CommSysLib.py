...
def svdMIMOencoder(input_stream, cc_gains, E_b=1):
    """ 
    Function to perform SVD-based precoding for a 2 x 2 MIMO Rayleigh Channel Model
    """
    output_stream = []
    idx = 0 
    for i in range(len(cc_gains)):
        # SVD 
        H = cc_gains[i]
        _, _, Vh = np.linalg.svd(H)
        V = np.conjugate(Vh).T
        # Extract two bits and BPSK map them
        s = input_stream[idx:idx+2]
        idx +=2 
        s = np.where(s == 1, E_b, -1 * E_b)
        # Transmit beamforming
        x = np.matmul(V, s)
        output_stream.append(x)

    return np.array(output_stream)

def svdMIMOdecoder(input_stream, cc_gains):
    """ 
    Function to perform svd-bassed decoding for a Rayleigh Channel model and 2 x 2 system
    """
    estimated_msg = []
    for i in range(len(cc_gains)):
        # y = Hx + w
        ...
        y = input_stream[i]
        # Receive beamforming
        y_hat = np.matmul(Uh, y)
        est_block = np.where(np.real(y_hat) >=0, 1, 0)
        estimated_msg.append(est_block)
    return np.array(estimated_msg).flatten()
